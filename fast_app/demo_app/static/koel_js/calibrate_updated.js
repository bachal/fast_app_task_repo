...
$('#zoom-out').click(function() {if(zoomLevel>0.60){updateZoom(-0.1);}});
zoomLevel = 0.60;
...
var map;
var beakons_data;
var selected_marker_lat;
var selected_marker_lng;
...
var live_marker_array1=[]
var d=0;

function add_new_marker(e){

       // console.log(e);
       // console.log(typeof(e.latlng));
        if(document.getElementById("msg").innerText == "Calibrate process started"){
                given_lat=e.latlng['lat']
                given_lng=e.latlng['lng']
                selected_marker_lat=e.latlng['lat'];
                selected_marker_lng=e.latlng['lng'];
               // console.log("lat = "+given_lat);
               // console.log("lng = "+given_lng);
                var LeafIcon = L.Icon.extend({options: {iconSize:[40, 40]}});
                var greenIcon = new LeafIcon({ iconUrl: "{{ url_for('static', filename='red_loc.png')}}"});
                live_marker = L.marker([given_lat,given_lng], {icon: greenIcon})
                live_marker.addTo(map)
                //live_marker.addTo(myFeatureGroup)
                socket.emit("get_beakons");
                live_marker_array.push([given_lat,given_lng]);
                live_marker_array1.push(live_marker);
                for(i=0;i<live_marker_array.length-1;i++) {
                if(live_marker_array.length>1){
                    live_marker_array1[i].remove();
                ...
                for(i=0;i<live_marker_array.length-1;i++) {
                    if(live_marker_array.length>1){
                        ...
                        var greenIcon = new LeafIcon({ iconUrl: "{{ url_for('static', filename='yellow_dots.png')}}"});
                        live_marker = L.marker(live_marker_array[i], {icon: greenIcon})
                        live_marker.addTo(map)
        ...
        if(document.getElementById("msg").innerText != "Calibrate process started"){
            ...
            document.getElementById("msg").innerText="Please first start calibrate process."
...
socket.on("get_beakons_response",function(beakons_data){
    document.getElementById('modal1').click();
    var str="";
    beakons_data_array=beakons_data;
    for(var i=0;i<beakons_data.length;i++){

        
        str+='<tr>';
        str+='<td>'+(i+1)+'</td>';
        str+='<td id="beakon_id'+String(i)+'">'+beakons_data[i]['Beacon ID']+'</td>';
        str+='<td id="beakon_power'+String(i)+'">'+beakons_data[i]['RSSI']+'</td>';
        str+='</tr>';
...
function start_calibrate(){
    ...
    if(document.getElementById("name").value!=""){
        users=document.getElementById("name").value
        start_dt['name']=document.getElementById("name").value
        start_dt['cal_dates']=document.getElementById("dates").value
        document.getElementById("msg").innerText = "Calibrate process started";
        document.getElementById("play_job").style.pointerEvents="none";
       // console.log(start_dt);
        socket.emit("start_cal_process",start_dt);
        socket.emit("start_time_calibrate")
    }
    if(document.getElementById("name").value==""){
        document.getElementById("msg").innerText = "Please enter name in start process";
    }
}

function calibrate(){
        calibrate_data={}
        calibrate_data['selected_location_lat']=selected_marker_lat
        calibrate_data['selected_location_lng']=selected_marker_lng
        calibrate_data['beakons_data']=beakons_data_array
        calibrate_data['user_name']=users
       // console.log("given_cal_dt");
        //console.log(calibrate_data);
        socket.emit("calibrated_data",calibrate_data);
...
function get_end_time(){
        if(document.getElementById("msg").innerText=='Calibrate process started'){
            socket.emit("end_time_calibrate")

            document.getElementById("msg").innerText = "Job end";
            document.getElementById("play_job").style.pointerEvents="none";
        }
        else{
            document.getElementById("msg").innerText = "Calbrate process not end please first start then end";
...
socket.on("start_time_response",function(time){
       ...
       document.getElementById("start_time").innerHTML=result[1];
       document.getElementById("dates").value=result[0];


});
socket.on("end_time_response",function(time){
        var result = time.split("-");
        document.getElementById("end_time").innerHTML=result[1];
});



function get_calibrate_start(){
