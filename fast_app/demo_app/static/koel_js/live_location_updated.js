...
//socket.emit("req_map");
socket.emit("get_live_loc");
socket.on("received_live_loc",function(given_live_data){
    document.getElementById("msg").innerText="Total Active Users :- " + String(given_live_data.length);
    
    get_dt=given_live_data[0]
    for(key in myFeatureGroup._layers){
            if(myFeatureGroup._layers[key]['test']==get_dt['SV ID']){
...
add_live_loc(given_lat,given_lng,get_dt['Live_Location'])
