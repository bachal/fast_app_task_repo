...
//alert("hello");
console.log("ok get list here");
console.log(main_list);
main_array = main_list.filter(function(e){return e});

for(var i=0;i<main_array.length;i++){

    for (var k=0;k<given_all_id.length;k++){

        if(main_array[i]['Part Number']==given_all_id[k]['Item']){
            main_array[i]['compared_part']=given_all_id[k]['Item']
            main_array[i]['SV ID']=given_all_id[k]['SV ID']
            main_array[i]['live_loction']=given_all_id[k]['Live_Location']
            main_array[i]['loction']=given_all_id[k]['Location']
            main_array[i]['marker_positions']=given_all_id[k]['marker_position']
            lat_lng=given_all_id[k]['marker_position']
            part_locations=given_all_id[k]['Live_Location']
           // add_live_loc(lat_lng[1],lat_lng[0],part_locations);
...
console.log("updated final array");
//console.log(main_array);
final_array = main_array.filter(function(e){return e});
console.log(final_array);

for(var j=0;j<main_array.length;j++){
    //console.log(main_array[j]);
    lat_lng=main_array[j]['marker_positions']
    part_locations=main_array[j]['live_loction']
   // add_live_loc(lat_lng[1],lat_lng[0],part_locations);
    //console.log("okkk");
...
for(key in myFeatureGroup._layers){
    for(var j=0;j<main_array.length;j++){
            if(myFeatureGroup._layers[key]['test']==main_array[j]['SV ID']){
                lat_lng=myFeatureGroup._layers[key]['_latlng'];
                given_lat=lat_lng['lat']
                given_lng=lat_lng['lng']
                add_live_loc(given_lat,given_lng,main_array[j]['live_loction'])
...
function add_live_loc(x,y,z){
               ...
               // var greenIcon = new LeafIcon({iconUrl: 'http://maps.google.com/mapfiles/ms/icons/red-dot.png'});
                var greenIcon = new LeafIcon({iconUrl: 'https://www.pngkit.com/png/full/126-1263730_blue-dot-png-graphic-download-circle.png'});
               ...
               //console.log("total_markers"+ myFeatureGroup.length)
